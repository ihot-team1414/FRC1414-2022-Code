...
public class AutoShoot extends CommandBase {
  ...
  public AutoShoot(ShooterSubsystem shooterSubsystem, IndexerSubsystem indexerSubsystem, HoodSubsystem hoodSubsystem) {
  ...
  public void execute() {
    ...
    if (!withinError && shooterSubsystem.isWithinAllowedError()) {
      ...
      withinError = false;
    } else if (withinError && Timer.getFPGATimestamp() - startTime > 0.3) {
      this.indexerSubsystem.load();
      SmartDashboard.putBoolean("hOLDING", false);
    } else {
      this.indexerSubsystem.stop();
